...
const menuItems = [
  ...
  { icon: <FaUser />, label: 'Players', link: '/Admin_Players' },
  { icon: <FaTrophy />, label: 'Tournaments', link: '/Admin_Tournaments' },
  { icon: <FaUsers />, label: 'Teams', link: '/Admin_Teams' },
  { icon: < RiProgress6Line />, label: 'Player Progress', link: '/Admin_Player_Progress' },
  { icon: <FaCalendarAlt />, label: 'Schedule', link: '/Admin_Schedule' },
];
  ...
  return (
    <div className="flex h-screen">
      ...
      <aside className={`fixed inset-y-0  left-0 bg-rgba-36-40-51-1 p-4 m-1 rounded-[18px] flex flex-col justify-between ${isSidebarOpen ? '' : 'hidden md:flex'}`}>
        ...
        <Link to="/" className="bg-white text-rgba-36-40-51-1 font-custom hover:bg-slate-300 text-center text-xl font-bold py-1 lg:py-2 rounded">
      Logout
    </Link>
      </aside>
