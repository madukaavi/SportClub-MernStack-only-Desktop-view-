...
const menuItems = [
  { icon: <FaChartBar />, label: 'Dashboard', link: '/PlayerDashboard' },
  { icon: <FaUser />, label: 'Player Stats', link: '/PlayerStats' },
  { icon: <FaTrophy />, label: 'Tournaments', link: '/Tournaments' },
  { icon: <FaUsers />, label: 'Teams', link: '/Teams' },
  { icon: <FaHistory />, label: 'Payment History', link: '/paymenthistory' },
  ...
  return (
    <div className="flex h-screen">
      ...
      <aside className={`fixed inset-y-0  left-0 bg-rgba-36-40-51-1 p-4 m-1 rounded-[18px] flex flex-col justify-between ${isSidebarOpen ? '' : 'hidden md:flex'}`}>
        ...
        <Link to="/" className="bg-white text-center text-rgba-36-40-51-1 font-custom hover:bg-slate-300 text-xl font-bold py-1 lg:py-2 rounded">
      Logout
    </Link>
      </aside>
